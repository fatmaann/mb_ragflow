...
class UserGroup(DataBaseModel):
    """User groups for RBAC - groups can have permissions on knowledge bases"""
    id = CharField(max_length=32, primary_key=True)
    ...
    is_admin = BooleanField(default=False, help_text="Admin group has full access to all resources", index=True)
    status = CharField(max_length=1, null=True, help_text="is it valid (0: invalid, 1: valid)", default="1", index=True)
...
class UserGroupMember(DataBaseModel):
    """Many-to-many relationship: Users belong to Groups"""
    id = CharField(max_length=32, primary_key=True)
    user_id = CharField(max_length=32, null=False, index=True, help_text="FK to user")
    group_id = CharField(max_length=32, null=False, index=True, help_text="FK to user_group")
    status = CharField(max_length=1, null=True, help_text="is it valid (0: invalid, 1: valid)", default="1", index=True)
...
class GroupKnowledgebasePermission(DataBaseModel):
    """Permissions for a Group on a specific Knowledgebase"""
    id = CharField(max_length=32, primary_key=True)
    group_id = CharField(max_length=32, null=False, index=True, help_text="FK to user_group")
    kb_id = CharField(max_length=32, null=False, index=True, help_text="FK to knowledgebase")
    can_read = BooleanField(default=False, help_text="Can view KB and its contents")
    can_update = BooleanField(default=False, help_text="Can update KB settings, documents, chunks")
    can_delete = BooleanField(default=False, help_text="Can delete KB, documents, chunks")
    can_create = BooleanField(default=False, help_text="Can add documents, chunks to KB")
    status = CharField(max_length=1, null=True, help_text="is it valid (0: invalid, 1: valid)", default="1", index=True)

    class Meta:
        db_table = "group_kb_permission"


# Default group names (constants for use in code)
DEFAULT_ADMIN_GROUP_NAME = "Administrators"
