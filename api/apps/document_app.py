...
from api.utils.rbac import require_kb_permission
...
async def doc_infos():
    ...
    for doc_id in doc_ids:
        ...
        if not PermissionService.can_read_kb(current_user.id, doc.kb_id):
            return get_json_result(data=False, message="No authorization.", code=RetCode.FORBIDDEN)
...
async def change_status():
    ...
    for doc_id in doc_ids:
        e, doc = DocumentService.get_by_id(doc_id)
        if not e or not PermissionService.can_update_kb(current_user.id, doc.kb_id):
...
async def rm():
    ...
    for doc_id in doc_ids:
        ...
        if not e:
            return get_json_result(data=False, message="Document not found.", code=RetCode.DATA_ERROR)
        if not PermissionService.can_delete_kb(current_user.id, doc.kb_id):
            return get_json_result(data=False, message="No authorization.", code=RetCode.FORBIDDEN)
    ...
    try:
        def _run_sync():
            for doc_id in req["doc_ids"]:
                e, doc = DocumentService.get_by_id(doc_id)
                if not e or not PermissionService.can_create_in_kb(current_user.id, doc.kb_id):
                    return get_json_result(data=False, message="No authorization.", code=RetCode.FORBIDDEN)
...
async def change_parser():
    ...
    if not e:
        return get_data_error_result(message="Document not found!")
    if not PermissionService.can_update_kb(current_user.id, doc.kb_id):
        return get_json_result(data=False, message="No authorization.", code=RetCode.FORBIDDEN)
