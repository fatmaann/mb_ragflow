...
from api.utils.rbac import require_admin, require_kb_permission, is_admin, get_accessible_kbs
from api.db.services.rbac_service import PermissionService
...
async def update():
    ...
    # Permission check is handled by @require_kb_permission('update') decorator
    try:
...
def detail():
    ...
    try:
        # Permission check is handled by @require_kb_permission('read') decorator
...
async def list_kbs():
    ...
    try:
        # Admins can see all KBs, regular users only see KBs they have read access to
        if is_admin(current_user.id):
            # Admin: use existing tenant-based logic
            if not owner_ids:
            ...
            # Non-admin: filter by RBAC permissions
            accessible_kb_ids = set(get_accessible_kbs(current_user.id, 'read'))
            if not owner_ids:
                tenants = TenantService.get_joined_tenants_by_user_id(current_user.id)
                tenants = [m["tenant_id"] for m in tenants]
...
@login_required
@require_kb_permission('delete')
@validate_request("kb_id")
async def rm():
    req = await get_request_json()
    # Permission check is handled by @require_kb_permission('delete') decorator
    try:
...
@require_kb_permission('read')
def list_tags(kb_id):
    # Permission check handled by @require_kb_permission('read') decorator
...
def list_tags_from_kbs():
    ...
    # Check read permission for all KB IDs
    for kb_id in kb_ids:
        if kb_id and not PermissionService.can_read_kb(current_user.id, kb_id):
            return get_json_result(
                data=False,
...
async def rm_tags(kb_id):
    req = await get_request_json()
    # Permission check handled by @require_kb_permission('update') decorator
...
def delete_knowledge_graph(kb_id):
    # Permission check handled by @require_kb_permission('delete') decorator
    _, kb = KnowledgebaseService.get_by_id(kb_id)
...
async def run_graphrag():
    ...
    # Permission check handled by @require_kb_permission('create') decorator
