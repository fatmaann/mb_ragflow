...
async def create():
    ...
    if not e:
        return get_data_error_result(message="Document not found!")
    if not PermissionService.can_create_in_kb(current_user.id, doc.kb_id):
        return get_json_result(data=False, message="No authorization.", code=RetCode.FORBIDDEN)
...
async def retrieval_test():
    ...
    for kb_id in kb_ids:
        if not PermissionService.can_read_kb(current_user.id, kb_id):
        ...
        for kb_id in kb_ids:
            for tenant in tenants:
                if KnowledgebaseService.query(
                    ...
                    break
            # Permission already checked above via RBAC
