...
async def set_dialog():
    ...
    # Check read permission on all KBs attached to this dialog
    kb_ids = req.get("kb_ids", [])
    if kb_ids:
        if not PermissionService.can_access_dialog_kbs(current_user.id, kb_ids):
            return get_json_result(
                data=False,
                message="You don't have read access to one or more knowledge bases in this dialog.",
...
def get():
    ...
    try:
        ...
        if not e:
            return get_data_error_result(message="Dialog not found!")
        # Check that the user owns this dialog or has access to its KBs
        if dia.tenant_id != current_user.id:
